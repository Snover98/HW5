...
struct Expression : public Node {
    ...
    Expression(const ExpType exp_type, const int used_register)
            : Node(), exp_type(exp_type), used_register(used_register), truelist(std::vector<int>()),
              falselist(std::vector<int>()) {}
};
...
struct Statement : public Node {
    ...
    Statement() : Node(), nextlist(std::vector<int>()), breaklist(std::vector<int>()) {}
};
...
struct StructMemory : public Node {
    ...
    StructMemory(const std::string &ID, VarType type, int line) : field(
            std::pair<std::string, VarType>(std::string(ID), type)), line(line) {}
};
